...
def test(content):
    ...
    for data in datas:
        ...
        cause1 = ''.join([word.split('/')[0] for word in data['cause'].split(' ') if word.split('/')[0]])
        tag1 = data['tag']
        effect1 = ''.join([word.split('/')[0] for word in data['effect'].split(' ') if word.split('/')[0]])
        return cause1, tag1, effect1
    # print(cause1)
    # print(tag1)
    # print(effect1)

# content = "既然我们是学生，就要遵守学生的基本规范。"
# cause, tag, effect = test(content)
# print(cause)
# print(tag)
# print(effect)